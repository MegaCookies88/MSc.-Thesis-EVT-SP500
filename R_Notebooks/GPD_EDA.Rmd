...
- Red Line : 95% marginal quantiles of losses (Original and Transformed to Frechet scale).
- Blue Line : 95% marginal quantiles of the data plotted in the right figure.
...
fq1 = frechet_scale(5,fit1$mle[2],fit1$mle[1],fit1$rate)
fq2 = frechet_scale(5,fit2$mle[2],fit2$mle[1],fit2$rate)

plot(x=x1, y=x2, xlab="Technology", ylab="Energy", log="xy")
abline(v=quantile(x1, 0.95, na.rm=TRUE), col=c1)
abline(h=quantile(x2, 0.95, na.rm=TRUE), col=c1)
abline(v=fq1, col=c2, untf=TRUE)
abline(h=fq2, col=c2, untf=TRUE)
abline(a=fq2, b=-fq2/fq1, col=c3, untf=TRUE)
...
x1 = -1/log(fn1(ll$S.P.Technology))
q1 = quantile(ll$S.P.Technology, 0.95)

fn2 = ecdf(ll$S.P.Energy)
x2 = -1/log(fn2(ll$S.P.Technology))
q2 = quantile(ll$S.P.Energy, 0.95)

par(pty="s")
plot(x=x1, y=x2, xlab="Technology", ylab="Energy", log="xy")
abline(v=-1/log(fn1(q1)), col=c2, untf=TRUE)
abline(h=-1/log(fn2(q2)), col=c2, untf=TRUE)
title(main="Technology/Energy - Empirical Frechet Scale")
```
...
tcplot(fla, u_fla, nt=100, pscale=TRUE, which=2, vci=FALSE, cilty=2, type="l", 
       ylim=c(-0.2,1.2), ylab="Tail Dependence", xlab="q")
abline(h=c(0,1), col=c2)
par(oma=c(0,0,0,0))
title(main="MLE Tail Dependence Nu - 95% CI")
```
