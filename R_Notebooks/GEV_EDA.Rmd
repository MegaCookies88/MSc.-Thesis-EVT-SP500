...
colnames(cml) = colnames(ll[,c(6,7)])

par(pty="s")
plot(ll[,c(6,7)], main="Original And Componentwise Maxima Data")
points(cml, col=c1)

ecml <- -1/log(apply(cml,2,rank)/202)
...
legend(
  ...
  col=c(c1,'black','black',c2), lty=c(1,2,3,1), cex=0.6
)
...
```{r}
par(pty="s")
plot(ll[,c(6,7)], main="Quantile Curves")
points(cml, col=c1)
...
m2 = fbvevd(cml, asy1 = 1, model = "alog")
...
```{r}
round(rbind(fitted(m1), std.errors(m1)), 3)
```
